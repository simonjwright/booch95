...
package body BC.Containers.Bags.Dynamic is
  ...
  procedure Purge (B : in out Dynamic_Bag) is
  begin
    Tables.Clear (B.Rep);
  end Purge;
  ...
  function Cardinality (B : Dynamic_Bag) return Natural is
  begin
    return Tables.Extent (B.Rep);
  end Cardinality;
  ...
  function Exists (B : Dynamic_Bag; I : Item) return Boolean is
  begin
    return Tables.Is_Bound (B.Rep, I);
  end Exists;

  function Value_Of (B : Dynamic_Bag; I : Item) return Positive is
  begin
    raise Program_Error;
    return 1;
  end Value_Of;
