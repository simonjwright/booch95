...
package body BC.Graphs is
   ...
   procedure Initialize (V : in out Vertex_Node) is
   begin
      Vertex_Id := Vertex_Id + 1;
      V.Id := Vertex_Id;
   end Initialize;
   ...
   procedure Adjust (V : in out Vertex_Node) is
   begin
      Vertex_Id := Vertex_Id + 1;
      V.Id := Vertex_Id;
   end Adjust;
   ...
   begin
      Ada.Text_IO.Put_Line ("Vertex_Node"
                            & Integer'Image (V.Id)
                            & " finalized.");
      if V.Count > 1 then
   ...
   procedure Initialize (A : in out Arc_Node) is
   begin
      Arc_Id := Arc_Id + 1;
      A.Id := Arc_Id;
   end Initialize;
   ...
   procedure Adjust (A : in out Arc_Node) is
   begin
      Arc_Id := Arc_Id + 1;
      A.Id := Arc_Id;
   end Adjust;
