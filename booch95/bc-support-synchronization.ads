...
package BC.Support.Synchronization is
  ...
  -- blocked until the owning task has Released the semaphore as many
  -- times as it Seized it.
  ...
  function None_Pending (On_The_Semaphore : Recursive_Semaphore) return Boolean;
  ...
  -- Multiple_Monitors allow multiple readers; however, when a writer
  -- owns the monitor it has exclusive access.
