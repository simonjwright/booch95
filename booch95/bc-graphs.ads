...
private
   ...
   type Vertex_Node is new Ada.Finalization.Controlled with record
      ...
      Count : Natural;
      Id : Integer;                  -- XXX finalization check
   end record;
   Vertex_Id : Integer := 0;         -- XXX finalization checks
   procedure Clear_Vertex_Node (G : in out Abstract_Graph'Class;
                                N : in out Vertex_Node_Ptr);
   procedure Initialize (V : in out Vertex_Node);
   procedure Adjust (V : in out Vertex_Node);
   ...
   Arc_Id : Integer := 0;            -- XXX finalization checks
   procedure Initialize (A : in out Arc_Node);
   procedure Adjust (A : in out Arc_Node);
