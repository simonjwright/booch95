...
package body Sequential_Fixed_Block is
   ...
   procedure Place (Value : in SSE.Storage_Offset;
                    Location : in System.Address) is
   begin
   ...
   begin
      if Pool.Next_Free /= Null_Pointer then
   ...
   begin
      Pool.Aligned_Element_Size :=
        ((Pool.Element_Size + Pool.Alignment - 1) / Pool.Alignment)
        * Pool.Alignment;
      if (SSE.Storage_Offset'Size / System.Storage_Unit)
        > Pool.Aligned_Element_Size then
         Raise_Exception
           (Representation_Error'Identity,
            "Bookkeeping would overlap allocated elements.");
      end if;
      if Pool.Content'Address mod Pool.Alignment /= 0 then
         --  Content(1) is not properly aligned
         Pool.Content_Alignment_Offset :=
           Pool.Alignment - (Pool.Content'Address mod Pool.Alignment);
      else
         --  Content(1) is aligned properly
