...
package body BC.Support.Dynamic is
   ...
   procedure Delete_Arr is new Ada.Unchecked_Deallocation(Dyn_Arr,Dyn_Arr_Ref);
   procedure Delete_Node is new Ada.Unchecked_Deallocation(Dyn_Node,Dyn_Node_Ref);
   ...
   procedure Insert (Obj : in out Dyn_Node; Elem : Item; Before : Natural) is
   begin
   ...
   procedure Append (Obj : in out Dyn_Node; Elem : Item; After : Natural) is
   begin
   ...
   procedure Remove (Obj : in out Dyn_Node; From : Natural) is
   begin
   ...
   function Location(Obj : access Dyn_Node; Elem : Item; Start : Positive := 1) return Natural is
   begin
      pragma Assert(Start <= Obj.Size);
      for i in Start..Obj.Size loop
         if Obj.Ref(i) = Elem then return i; end if;
      end loop;
