...
package body Tests.Indefinite_Maps is
   ...
   procedure Initially_Empty (C : in out Test_Case'Class) is
      ...
      M, N : Map;
   begin
      Assert (Extent (M) = 0, "Extent (M) should be 0");
      Assert (Is_Empty (M), "M should be empty");
      Assert (M = Null_Container, "M should be equal to Null_Container");
      Assert (Extent (N) = 0, "Extent (N) should be 0");
      Assert (Is_Empty (N), "N should be empty");
      Assert (N = Null_Container, "N should be equal to Null_Container");
   ...
   begin
      Clear (N);
      Assert (Extent (N) = 0, "Extent (N) should be 0 after Clear");
      Assert (Is_Empty (N), "N should be empty after Clear");
      Assert (N = Null_Container,
              "N should be equal to Null_Container after Clear");
      Assert (M = N, "M should be equal to N after Clear");
      Clear (N);
      Assert (Extent (N) = 0, "Extent (N) should be 0 after second Clear");
      Assert (Is_Empty (N), "N should be empty after second Clear");
      Assert (N = Null_Container,
              "N should be equal to Null_Container after second Clear");
      Assert (M = N, "M should be equal to N after second Clear");
   ...
   begin
      ...
      declare
         ...
         begin
            ...
            case Positive'Value (K) is
               when 1 => Assert (I = "a", "I for K=""1"" should be ""a""");
               when 2 => Assert (I = "b", "I for K=""2"" should be ""b""");
               when 3 => Assert (I = "c", "I for K=""3"" should be ""c""");
               when 4 => Assert (I = "d", "I for K=""4"" should be ""d""");
               when 5 => Assert (I = "e", "I for K=""5"" should be ""e""");
               when others =>
                  Assert (False, "K should be in ""1""..""5""");
            end case;
         end Apply;
         It : Map_Iterator'Class := Map_Iterator (New_Iterator (M));
      begin
         Visitor (It);
         Assert (Count = 5, "wrong number of items");
      end;
      declare
         ...
         begin
            OK := True;
            if K = "3" then
               Assert (I = "c", "I for K=""3"" should be ""c""");
               I := "C";
               OK := False;
            end if;
         end Apply;
      ...
      begin
         ...
         Bind (N, "4", "d");
         Bind (N, "5", "e");
         Assert (M = N, "maps not equal after Modify");
      end;
      declare
         Count : Natural := 0;
         It : Map_Iterator'Class := Map_Iterator (New_Iterator (M));
      ...
      begin
         while not Is_Done (It) loop
            ...
            begin
               case K is
                  when 1 =>
                     Assert (Current_Item (It) = "a",
                             "Current_Item (""1"") should be ""a""");
                     Next (It);
                  when 2 =>
                     Assert (Current_Item (It) = "b",
                             "Current_Item (""2"") should be ""b""");
                     Next (It);
                  when 3 =>
                     Assert (Current_Item (It) = "C",
                             "Current_Item (""3"") should be ""C""");
                     Delete_Item_At (It);
                  when 4 =>
                     Assert (Current_Item (It) = "d",
                             "Current_Item (""4"") should be ""d""");
                     Next (It);
                  when 5 =>
                     Assert (Current_Item (It) = "e",
                             "Current_Item (""5"") should be ""e""");
                     Next (It);
                  when others =>
                     Assert (False, "Current_Key should be in ""1""..""5""");
               end case;
            end;
         end loop;
         Assert (Count = 5, "wrong number of items");
         ...
         Bind (N, "4", "d");
         Bind (N, "5", "e");
         Assert (M = N, "maps not equal after Delete_Item_At");
      end;
   end Iteration;
