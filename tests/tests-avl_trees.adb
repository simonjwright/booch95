...
package body Tests.AVL_Trees is
   ...
   begin
      ...
      Assert (C, Extent (T) = 0, "extent not 0");
      Assert (C, Is_Null (T), "tree not null");
      Assert (C, not Is_Member (T, Item'('a', 0)), "('a', 0) is a member");
      declare
         Found : Boolean := True;
      begin
         Delete (T, Item'('a', 0), Found);
         Assert (C, not Found, "('a', 0) found on deletion");
      end;
   end Initially_Empty;
   ...
   begin
      ...
      Assert (C, Not_Found, "('a', 0) was found");
      Assert (C,
              To_String (T) = "a",
              "expecting ""a"", got """ & To_String (T) & """");
      Assert (C, Extent (T) = 1, "extent not 1");
      Assert (C, not Is_Null (T), "tree is null");
      Assert (C, Is_Member (T, Item'('a', 0)), "('a', 0) is not a member");
      Assert (C, Is_Member (T, Item'('a', 1)), "('a', 1) is not a member");
      Assert (C, Is_Member (T, Item'('A', 0)), "('A', 0) is not a member");
      Assert (C, not Is_Member (T, Item'('b', 0)), "('b', 0) is a member");
      Assert (C, not Is_Member (T, Item'('B', 0)), "('B', 0) is a member");
      declare
         T2 : AVL_Tree := T;
         Found : Boolean := False;
      begin
         ...
         Assert (C, Found, "('a', 0) found on deletion");
         Assert (C,
                 Extent (T2) = 0,
      ...
      begin
         ...
         Assert (C, not Found, "('b', 0) found on deletion");
      end;
      Not_Found := True;
      Insert (T, Item'('a', 1), Not_Found);
      Assert (C, not Not_Found, "('a', 1) was not found");
      Assert (C, Extent (T) = 1, "extent not 1 after inserting ('a', 1)");
      Not_Found := True;
      Insert (T, Item'('A', 2), Not_Found);
      Assert (C, not Not_Found, "('A', 2) was not found");
      Assert (C, Extent (T) = 1, "extent not 1 after inserting ('A', 2)");
   end One_Element;
